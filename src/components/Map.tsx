    ...
    useEffect(() => {
        ...
        if (mapContainerRef.current && !mapRef.current) {
            try {
                ...
                // 5. Long Press to Navigate Interaction
                // Logic: Timer starts on touch/mousedown. If 800ms passes without movement/mouseup, trigger.
                ...
                const startLongPress = (e: any) => {
                    isLongPress = false;
                    startPoint = [e.point.x, e.point.y]; // Screen coords
                ...
                const checkMove = (e: any) => {
                    if (startPoint && longPressTimer) {
                        const dist = Math.sqrt(
                            Math.pow(e.point.x - startPoint[0], 2) +
                            Math.pow(e.point.y - startPoint[1], 2)
                        );
                        if (dist > 10) { // Tolerance (pixels)
                            cancelLongPress();
                        }
                    }
                };
